#!/usr/bin/env python3
"""
Blind Exploration: DFS (left) vs BFS / Random (right)
Subtle cell borders, clean split, fullscreen.
"""
...
# Cell border – very subtle, almost transparent appearance
CELL_BORDER = (80, 90, 110)          # muted grey
...
OUTLINE_COL = (120, 140, 180)        # grid outline
...
class BlindExplorer:
    ...
    def step(self):
        ...
        if self.strategy == 'dfs':
            next_cell = self._dfs_next()
        elif self.strategy == 'bfs':
            next_cell = self._bfs_next()
        elif self.strategy == 'random':
            next_cell = self._random_next()

        if next_cell is None:
            if self.parent.get(self.pos) is not None:
                next_cell = self.parent[self.pos]
            else:
                self.finished = True
                return
    ...
    def _bfs_next(self):
        ...
        for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
            nx, ny = x+dx, y+dy
            if 0 <= nx < self.cols and 0 <= ny < self.rows and self.grid[ny][nx] == 0:
                if (nx, ny) not in self.visited and (nx, ny) not in self.bfs_queue:
                    self.bfs_queue.append((nx, ny))
                    self.parent[(nx, ny)] = self.pos
        ...
        target = self.bfs_queue[0]
        if abs(target[0] - self.pos[0]) + abs(target[1] - self.pos[1]) == 1:
            self.bfs_queue.popleft()
            return target

        parent = self.parent.get(target)
        if parent is None or parent == self.pos:
            return None
    ...
    def _random_next(self):
        ...
        if neighbors:
            ...
            self.parent[chosen] = self.pos
            return chosen
        return self.parent.get(self.pos)
...
def main():
    ...
    pygame.display.set_caption("Blind Exploration – subtle cell borders")
    ...
    def reset_all():
        ...
        start, goal = auto_place_start_goal(grid)
        left_agent = BlindExplorer(grid, start, goal, 'dfs')
        right_agent = BlindExplorer(grid, start, goal, right_strategy.lower())
        left_frontier, left_visited, left_pos = left_agent.get_state()
        right_frontier, right_visited, right_pos = right_agent.get_state()
        animating = False
        running_search = False
    ...
    def run_both():
        ...
        right_agent.__init__(grid, start, goal, right_strategy.lower())
        left_frontier, left_visited, left_pos = left_agent.get_state()
        right_frontier, right_visited, right_pos = right_agent.get_state()
        animating = True
        running_search = False
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if run_btn.collidepoint(x, y):
                    ...
                    FRAME_DELAY_MS = max(50, FRAME_DELAY_MS - 25)
                    status = f"Delay: {FRAME_DELAY_MS} ms"
                elif speed_plus_btn.collidepoint(x, y):
                    FRAME_DELAY_MS = min(1000, FRAME_DELAY_MS + 25)
                    status = f"Delay: {FRAME_DELAY_MS} ms"
        ...
        for y in range(ROWS):
            for x in range(COLS):
                ...
                # Subtle border around the full cell (both halves together)
                # Use 1‑pixel border with a muted colour
                cell_rect = pygame.Rect(cx, cy, cell, cell)
                pygame.draw.rect(screen, CELL_BORDER, cell_rect, 1)

        # Outline around the whole grid
        ...
        left_stats = f"DFS: cost {left_agent.total_cost:.1f}, steps {left_agent.steps}"
